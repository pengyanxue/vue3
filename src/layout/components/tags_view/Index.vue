 ...
 * @LastEditTime: 2022-04-27 15:49:26
-->
<template>
  <div
  ...
  setup() {
    ...
    const state = reactive({
      ...
      refreshSelectedTag: (view) => {
        store.dispatch('ACTION_DEL_CACHED_VIEW', view)
        const { fullPath } = view
        nextTick(() => {
      ...
      closeSelectedTag: (view) => {
        store.dispatch('ACTION_DEL_VIEW', view)
      ...
      closeOthersTags: () => {
        ...
        store.dispatch('ACTION_DEL_OTHER_VIEW', state.selectedTag)
      },
      closeAllTags: (view) => {
        store.dispatch('ACTION_DEL_ALL_VIEWS', undefined)
    ...
    const initTags = () => {
      ...
      for (const tag of state.affixTags) {
        // Must have tag name
        if (tag.name) {
          store.dispatch('ACTION_ADD_VISITED_VIEW', tag)
        }
      }
    }
    ...
    const addTags = () => {
      if (currentRoute.name) {
        ...
        store.dispatch('ACTION_ADD_VIEW', currentRoute)
      }
      return false
    }
        ...
        for (const tag of tags) {
          if ((tag.to).path === currentRoute.path) {
            ...
            if ((tag.to).fullPath !== currentRoute.fullPath) {
              store.dispatch('ACTION_UPDATE_VISITED_VIEW', currentRoute)
            }
          }
        }
      })
